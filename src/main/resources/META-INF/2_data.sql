...
CREATE PROCEDURE populate_movie_table() -- Define stored procedure
BEGIN
  DECLARE count INT; -- Declare variable to store row count
  SELECT COUNT(*) INTO count FROM movie; -- Count existing rows

  IF count = 0 THEN -- Check if table is empty

    INSERT INTO movie (uuid, title, releaseYear, director, rating, genre)
    VALUES
      (UUID_TO_BIN(UUID()),'The Shawshank Redemption', 1994, 'Frank Darabont', 9.3, 'Drama'),
